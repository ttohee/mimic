...
import { SCENARIOS, TRANSCRIPTS } from '../lib/data';
...
export default function Transcript({ go: _go }: { go: (v: ViewType) => void }) {
  const [scen, setScen] = useState('all');
  const [sort, setSort] = useState<'recent' | 'score'>('recent');

  let list = TRANSCRIPTS.filter(t => scen === 'all' || t.scenario === scen);
  list = [...list].sort((a, b) => sort === 'recent' ? b.date.localeCompare(a.date) : b.score - a.score);
  ...
  return (
    <div style={{ padding: '34px 40px 60px', maxWidth: 880, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
        <select value={scen} onChange={e => setScen(e.target.value)} style={{ border: '1.5px solid var(--border-strong)', background: 'var(--surface)', borderRadius: 'var(--r-pill)', padding: '9px 16px', fontSize: 14, color: 'var(--text)', cursor: 'pointer' }}>
            ...
            <div key={t.id} className="card" style={{ padding: '18px 22px', borderRadius: 'var(--r-lg)', animation: `fadeUp .4s ${i * 0.04}s` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <button className="btn btn-soft btn-sm" onClick={() => download(t)}><Icon name="download" size={16} /> .txt</button>
                </div>
              </div>
